...
class FSUserInteractionView: UIView {
    
    // MARK: - Override Methods
    
    override func hitTest(_ point: CGPoint, with event: UIEvent?) -> UIView? {
        // 如果视图隐藏或用户交互被禁用，直接返回nil
        if isHidden {
            return nil
        }
...
open class FSPopoverView: UIView {
    ...
    public var customUserInteractionEnabled: Bool = true {
        didSet {
            setNeedsReload()
        }
    }
...
private extension FSPopoverView {
    ...
    func p_createContainerView() -> UIView {
        let view = FSUserInteractionView()
        view.backgroundColor = .clear
        return view
    }
    ...
    func p_setContainerView(_ view: UIView) {
        ...
        userInteractionView.isUserInteractionEnabled = customUserInteractionEnabled
    }
    ...
    func p_reloadData() {
        ...
        do {
            ...
            if autosetsArrowDirection {
                ...
                if topSpace.width >= verticalContentSize.width && topSpace.height >= verticalContentSize.height {
                    contentSize = verticalContentSize
                    arrowDirection = .down
                } else if bottomSpace.width >= verticalContentSize.width && bottomSpace.height >= verticalContentSize.height {
                   contentSize = verticalContentSize
                   arrowDirection = .up
