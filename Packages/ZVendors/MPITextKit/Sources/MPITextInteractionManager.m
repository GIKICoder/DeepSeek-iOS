...
- (void)interactiveAction:(MPITextInteractiveGestureRecognizer *)recognizer {
    ...
    if (state == UIGestureRecognizerStateBegan) {
     ...
     if (recognizer.result == MPITextInteractiveGestureRecognizerResultLongPress) {
          if (interactableView.isSelectable) {
              if (inSelection) {
                  ...
                  [self beginSelectionAtPoint:location withShowMenu:NO];
              }
          } else {
              [self longPressLinkIfNeeded];
          }
...
- (void)beginSelectionAtPoint:(CGPoint)point withShowMenu:(BOOL)showMenu
{
  [self.interactableView beginSelectionAtPoint:point withShowMenu:showMenu];
}
...
- (MPITextMagnifier *)rangedMagnifier {
    if (!_rangedMagnifier) {
        _rangedMagnifier = [MPITextMagnifier magnifierWithType:MPITextMagnifierTypeCaret];
        /// MPITextMagnifierTypeRanged
