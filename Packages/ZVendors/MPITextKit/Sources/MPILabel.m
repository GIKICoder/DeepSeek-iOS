...
- (void)copy:(nullable id)sender {
    ...
    if (!self.attributedText && self.textRenderer) {
        attributedText = self.textRenderer.renderAttributes.attributedText;
    } 
    NSString *string = [attributedText mpi_plainTextForRange:self.selectedRange];
    if (string.length > 0) {
...
{
  [self beginSelectionAtPoint:point withShowMenu:YES];
}
